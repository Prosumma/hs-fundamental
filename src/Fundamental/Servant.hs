...
runApplication ::
  forall (api :: Type) s.
  (HasServer api '[]) =>
  (forall a. ServerExceptionHandler s a) ->
  Proxy api ->
  ServerT api (RIO s) ->
  s ->
  Application
...
runApplicationWithContext ::
  forall (api :: Type) (context :: [Type]) s.
...
-- | Converts a function which returns `Maybe a` into
-- one which throws a 404 when `Maybe a` is `Nothing`.
